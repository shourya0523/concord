...
def write_answer_coverage_report(path: Path, m: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        *[f"| `{k}` | {v} |" for k, v in sorted(prov.items(), key=lambda kv: -kv[1])],
        ...
        "  human-reviewed; with an OpenRouter key, small-model `rubric-v1` drafts replace only the",
        "  ones that fail validation or fall below the auto-approve bar, and only when Jev verifies",
        "  the draft as supported by the teaching answer.",
        "",
        "## Provenance rules",
        "",
