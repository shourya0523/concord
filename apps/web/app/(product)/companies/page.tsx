...
import { InkHoverScope, WarrenCallout } from "@/components/paper"
...
function groupBySignalVolume(items: FirmCatalogItem[]): VolumeGroup[] {
  ...
  return [
    { id: "deep", label: "Deepest signal coverage", hint: "≥ half of the catalog peak", firms: deep },
    { id: "established", label: "Established coverage", hint: "meaningful reported volume", firms: established },
    { id: "early", label: "Early coverage", hint: "thin volume — treat heat as directional", firms: early },
...
function SignalMeter({ signals, max }: { signals: number; max: number }) {
  ...
  return (
    <span className="flex items-center gap-2" aria-label={`${signals} reported signals`}>
      <span
        aria-hidden
...
export default async function CompaniesIndexPage() {
  ...
  const max = catalog.items.reduce((peak, firm) => Math.max(peak, firm.signals), 0)
    ...
    <div className="space-y-10">
      <header className="space-y-3">
        ...
        <p className="max-w-2xl text-[15px] leading-relaxed text-muted-foreground">
          {catalog.items.length} firm rooms, ranked by how much reported interview volume sits
          behind each one. Open a room for topic heat, over-indexed concepts, and the reported
          signal browser.
        </p>
      </header>

      <WarrenCallout mood="idle">
        Every room here runs on Glassdoor <em>occurrence</em> signals — which topics show up and
        how often. They point you at what to drill; teaching answers always come from the corpus,
        never from scraped interview text.
      </WarrenCallout>

      {catalog.note ? (
        <p className="font-mono text-[11px] tracking-wide text-muted-foreground">{catalog.note}</p>
      ) : null}

      {catalog.items.length === 0 ? (
        <p className="border border-dashed border-border px-4 py-6 text-sm text-muted-foreground">
          No firm rooms are published yet. Run an occurrence import and the rooms will open here,
          ranked by signal volume.
        </p>
      ) : (
        <InkHoverScope selector="a" className="space-y-10">
          {groups.map((group) => (
            <section key={group.id} className="space-y-1" aria-label={group.label}>
                  ...
                  return (
                    <li key={firm.id}>
                      <Link
                        ...
                        className="flex flex-wrap items-baseline gap-x-4 gap-y-1 border-b border-stone/60 px-2 py-3 transition-colors duration-200 ease-out hover:bg-foreground/[0.03]"
                      >
